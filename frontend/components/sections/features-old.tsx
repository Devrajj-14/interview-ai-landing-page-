...
const Features = () => {
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      // Dynamic activation zone based on scroll speed and direction changes
      let activationZone = 200;
      if (wheelEventCount > 5 || directionChanges > 2) {
        activationZone = 350; // Very wide for rapid/erratic scrolling
      } else if (wheelEventCount > 3) {
        activationZone = 280; // Wide for fast scrolling
      }
      ...
      if (shouldCapture) {
        ...
        // Highly adaptive sensitivity
        let sensitivity = 3.5;
        if (directionChanges > 2) {
          // Rapid direction changes - very responsive
          sensitivity = 2.0;
        ...
        // Cancel any pending frame and request new one immediately
    ...
    const handleScroll = () => {
      ...
      // Reset if scrolled back up past the section
      if (rect.bottom < -300) {
        setInternalScroll(0);
        pendingScroll = 0;
        isLocked = false;
    ...
    // Use capture phase for immediate event interception
